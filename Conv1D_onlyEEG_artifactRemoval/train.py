...
def eeg_reject(eeg_mat, thresh=None):
    rej_mat = np.zeros_like(eeg_mat)

    for n, ch in enumerate(eeg_mat):
        if not thresh:
            rej_val = abs(ch).mean() + (2 * abs(ch).std())
        else:
            rej_val = thresh
        idx = np.where(abs(ch) > rej_val)
        rej_mat[n, idx] = 1

    rej_mat = 1 - rej_mat
    eeg_mat_rej = eeg_mat * rej_mat

    return eeg_mat_rej

# generate corresponding x and y
def xs_gen(data_set):
    eeg = np.array(eeg_reject(np.array(data_set['x_eeg'])))
