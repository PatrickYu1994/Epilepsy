...
def xs_gen(data_set):
    ...
    xs = np.array(list(map(lambda x: [x], xs))) # shape: 987, 1, 500, 17, dataformat is channel_first
...
def build_model(xs):
    ...
    model.add(Conv2D(64, (7, 7), strides=(2, 1), padding='same', activation='relu',
                     kernel_initializer='he_uniform', input_shape=(xs.shape[1], xs.shape[2], xs.shape[3])))
    ...
    model.add(Conv2D(128, (3, 3), strides=(2, 1), padding='same', activation='relu',
                     kernel_initializer='he_uniform'))
    model.add(BatchNormalization(axis=1))
    model.add(Conv2D(128, (3, 3), strides=(2, 1), padding='same', activation='relu',
                     kernel_initializer='he_uniform'))
